...
const TasteProfileReminderModal = () => {
  ...
  const handleCompleteNow = () => {
    // Play the exit animation first, then swap straight into the quiz modal
    setShow(false);
    setTimeout(() => {
      setVisible(false);
      setQuizOpen(true);
  ...
  return (
    <>
      ...
      <TasteQuizModal
        ...
        onClose={() => {
          setQuizOpen(false);
          sessionStorage.setItem(DISMISS_KEY, '1'); // once they've engaged with the quiz, don't re-nag this session
        }}
