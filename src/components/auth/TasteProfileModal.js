...
const EXPERIENCE_OPTIONS = ['beginner', 'intermediate', 'enthusiast', 'connoisseur'];
const DIETARY_OPTIONS = ['Sulfite-Free', 'Vegan', 'Organic', 'Low-Sugar'];
const FLAVOR_TAGS = ['black-fruit', 'red-fruit', 'citrus', 'spice', 'earthy', 'vanilla', 'oaky', 'floral', 'herbal', 'bitter', 'smoky'];
...
const TasteProfileModal = ({ isOpen, onClose, existingProfile }) => {
  ...
  // A flavor can't be both a like and a dislike at once
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(saveTasteProfile({
        ...
        typical_budget_per_bottle: { min: Number(form.budget_min) || 0, max: Number(form.budget_max) || 0 },
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <h2 className="text-2xl font-serif mb-1">Your Taste Profile</h2>
        <p className="text-xs text-zinc-400 mb-8">Help us recommend wines you'll actually love</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-3 gap-2">
              {TANNIN_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`py-2 text-[11px] font-bold uppercase tracking-wide border rounded-md transition-colors ${
            ...
            <div className="grid grid-cols-4 gap-2">
              {SWEETNESS_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`py-2 text-[10px] font-bold uppercase tracking-wide border rounded-md transition-colors ${
          ...
          <div>
            ...
            <p className="text-[11px] text-zinc-400 mb-3">Tap once to like, tap again to remove. Tap "dislike" toggle below to mark as disliked instead.</p>
              ...
              {FLAVOR_TAGS.map((tag) => {
                ...
                return (
                  <button
                    type="button"
                    key={tag}
                    onClick={() => toggleFlavor(isDisliked ? 'dislikes' : 'likes', tag)}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {DIETARY_OPTIONS.map((opt) => (
                <Pill key={opt} active={form.dietary_restrictions.includes(opt)} onClick={() => toggleInArray('dietary_restrictions', opt)}>
                  {opt}
                </Pill>
              ))}
            </div>
